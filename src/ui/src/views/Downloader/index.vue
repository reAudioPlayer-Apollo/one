...
const formOptions = ref<IOption[]>([
    {
        ...
        accept: "audio/mp3",
        required: true,
        onChange: async (src: unknown) => {
            const metadata = await fetchMetadata(src as string);
...
<template>
    <div class="downloader py-2 pr-2 grid gap-4 grid-cols-2 items-start">
        <div class="downloads">
            <template v-for="state in downloader.states" :key="state.song.id">
                <Card v-if="!state.internal" :class="[
                    'song',
                    {
                        downloading:
                            downloader.states[state.song.id ?? '']?.status ==
                            'downloading',
                    },
                    ...
                    <template v-if="state.status !== 'pending'">
                        ...
                        <aside>
                            <div class="wrap-progress">
                                <span class="material-symbols-rounded downloading" v-if="state.status == 'downloading'">
                                    south
                                </span>
                                <span class="material-symbols-rounded cursor-pointer finished"
                                    v-else-if="state.status == 'finished'" @click="downloader.download(state.song.id)">
                                    download_for_offline
                                </span>
                                <span class="material-symbols-rounded cursor-pointer downloaded"
                                    v-else-if="state.status == 'downloaded' && state.song.id"
                                    @click="reDownload(state.song.id)">
                                    download_done
                                </span>
                                <span class="material-symbols-rounded cursor-pointer error"
                                    v-else-if="state.status == 'error' && state.song.id"
                                    @click="reDownload(state.song.id)">
                                    error
                                </span>
                            </div>
                    ...
                    <div v-else class="col-span-2 flex flex-row justify-center items-center">
                        <Loader />
                    </div>
                </Card>
            </template>
        ...
        <div class="new-download">
            <Card class="custom p-4 flex flex-col">
                ...
                <IconButton icon="download_for_offline" label="Download" class="w-full mt-8" @click="requestDownload" />
            </Card>
        </div>
    </div>
</template>
