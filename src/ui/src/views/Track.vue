...
const load = async (spotifyId: string = null) => {
    ...
    metadata.value = await getSongMetadata(
        song.value.id,
        !!spotifyId,
        spotifyId
    );
...
watch(spotifyUrl, () => {
    if (
        metadata.value?.spotify?.id == parseSpotifyId(spotifyUrl.value, "track")
    ) {
        spotifyUrlIcon.value = "link";
        return;
    }
                        ...
                        <div class="trac__info__details__normal">
                            ...
                            <div class="flex flew-row items-center">
                                <span
                                    ...
                                    @click="
                                        player.loadPlaylist('track', song.id)
                                    "
                                >
                                    play_circle
                                </span>
                            ...
                            >
                                <FactCard
                                    ...
                                    :primary-text="
                                        ...
                                        ' ' +
                                        metadata.spotify.features.mode
                                    "
                                    class="w-full"
                                    secondary-text="Key"
                                />
                                ...
                                <ButtonCard
                                    icon="download"
                                    label="Download"
                                    @click="downloadSong(song.id)"
                                />
                            </div>
                            <div class="spotify-infos mt-4">
                                <div class="meta items-center">
                                    <span class="text-muted">{{
                                        localeDate(metadata.spotify.releaseDate)
                                    }}</span>
                                    <span
                                        v-if="metadata.spotify.explicit"
